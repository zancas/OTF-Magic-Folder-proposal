...
Open Source, so that people who are blocked from accessing Dropbox can deploy
Magic Folders using only their own local resources.

There are already tools available to people in repressive
...
and vulnerable people under repressive regimes, both internally to ease the
...
extensively by professional human rights defenders, by activists, and by
...
We propose to extend the Tahoe-LAFS storage system into an easy-to-use
...
scientists, Richard Stallman, the Electronic Frontier Foundation,
